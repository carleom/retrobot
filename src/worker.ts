...
export interface WorkerData {
  input: InputState;
  duration: number;
  steps?: Array<{ input: InputState; duration: number }>; // batched execution
  coreType: CoreType;
  game: Buffer;
  state: Buffer;
  gameHash?: string;
...
export default async (data: WorkerData) => {
  let { coreType, input, duration, game, state, gameHash, stateHash } = data;
  ...
  const incomingGameHash = gameHash ? gameHash : await crc32c(game);
  ...
  {
    const incomingStateHash = stateHash ? stateHash : await crc32c(state);
  ...
  core.retro_set_input_state(
    (port: number, device: number, index: number, id: number) => {
      if (id == RETRO_DEVICE_ID_JOYPAD_MASK) {
        let mask = 0;

        if (input.A) mask |= 1 << RETRO_DEVICE_ID_JOYPAD_A;

        if (input.B) mask |= 1 << RETRO_DEVICE_ID_JOYPAD_B;

        if (input.X) mask |= 1 << RETRO_DEVICE_ID_JOYPAD_X;

        if (input.Y) mask |= 1 << RETRO_DEVICE_ID_JOYPAD_Y;

        if (input.SELECT) mask |= 1 << RETRO_DEVICE_ID_JOYPAD_SELECT;

        if (input.START) mask |= 1 << RETRO_DEVICE_ID_JOYPAD_START;

        if (input.UP) mask |= 1 << RETRO_DEVICE_ID_JOYPAD_UP;

        if (input.DOWN) mask |= 1 << RETRO_DEVICE_ID_JOYPAD_DOWN;

        if (input.LEFT) mask |= 1 << RETRO_DEVICE_ID_JOYPAD_LEFT;

        if (input.RIGHT) mask |= 1 << RETRO_DEVICE_ID_JOYPAD_RIGHT;

        if (input.L) mask |= 1 << RETRO_DEVICE_ID_JOYPAD_L;

        if (input.R) mask |= 1 << RETRO_DEVICE_ID_JOYPAD_R;
  ...
  core.retro_set_video_refresh(
    (data: number, width: number, height: number, pitch: number) => {
      callback({
        buffer: data
          ? new Uint16Array(
              core.HEAPU16.subarray(data / 2, (data + pitch * height) / 2),
            )
          : null,
        width,
        height,
  ...
  const frames: Frame[] = [];
  const allSteps = data.steps || [{ input, duration }];
  for (const step of allSteps) {
    // Update the captured input for this step's duration
    input = step.input;
